 ...
 */
export async function getUseCaseWithProjectContext(args) {
    ...
    try {
        ...
        if (!projectId) {
            return text(`Use case "${useCaseId}" has no projectId — cannot retrieve project context.\n\n` +
                `Use Case: ${useCase.key} - ${useCase.name}`);
        }
        const project = await projectService.getProjectById(projectId);
        if (!project) {
            return text(`Project "${projectId}" referenced by use case "${useCaseId}" was not found.\n\n` +
                `Use Case: ${useCase.key} - ${useCase.name}`);
        }
        ...
        return text(`Error retrieving use case with project context: ` +
...
//# sourceMappingURL=getUseCaseWithProjectContext.js.map