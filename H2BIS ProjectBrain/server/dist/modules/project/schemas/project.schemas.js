...
export const DomainModelFieldSchema = z.object({
    ...
    type: z
        .string()
        .min(1)
        .describe('TypeScript-style type string: "string", "number", "boolean", "Date", "string[]", "ObjectId", or any custom class name'),
    required: z
        .boolean()
        .optional()
        .default(false)
...
export const upsertProjectDomainModelSchema = z.object({
    ...
    name: z
        .string()
        .min(1)
        .describe('Domain model name (e.g. "UserDocument", "ProjectDto"). Case-sensitive — acts as the unique key for upsert.'),
    description: z
        .string()
        .optional()
    ...
    layer: z
        ...
        .optional()
        .describe('Architectural layer: data=DB document, dto=transport, domain=domain object, response/request=API shapes, event=async message, other.'),
    fields: z
...
export const removeProjectDomainModelSchema = z.object({
    ...
    modelName: z
        .string()
        .min(1)
        .describe('Exact name of the domain model to remove (case-sensitive, e.g. "UserDocument")'),
});
